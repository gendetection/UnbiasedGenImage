...
group.add_argument('generator', type=str, choices=['Midjourney', 'stable_diffusion_v_1_5', 'stable_diffusion_v_1_4', 'wukong', 'ADM', 'VQDM', 'glide', 'BigGAN'],
...
group.add_argument('--balance_train_classes', action='store_true', default=False,
...
group.add_argument('--min_size', type=int, default=None,
                    help='Only nature images in intervall [min_size, max_size] are included')
group.add_argument('--max_size', type=int, default=None,
                    help='Only nature images in intervall [min_size, max_size] are included')
...
def main():
    ...
    if args.dataset.lower() == "SIZE_CONSTRAINED".lower():
        assert args.min_size and args.max_size
    ...
    if args.apex_amp and has_apex:
        ...
        _logger.warning("Neither APEX or native Torch AMP is available, using float32. "
                        "Install NVIDA apex for upgrade to PyTorch 1.6")
    ...
    data_train, data_val = get_data(args, is_training=True, balance_val_classes=args.balance_val_classes, balance_train_classes=args.balance_train_classes)
    pre_transform = get_transform(args, data_train)
    dataset_train = create_dataset(data_train, pre_transform, args.class_map)
    dataset_eval = create_dataset(data_val, pre_transform, args.class_map)
